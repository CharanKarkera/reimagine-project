  ...
  return (
    <nav className="flex flex-col justify-center text-xl items-center text-[ivory] radial-gradient">
      ...
      <div className="flex justify-between items-center w-full relative">
        <img src="/logo.png" alt="Meesho" className="order-1 h-[80px] w-[220px]   p-5 hover:scale-105 transition-transform duration-300 ease-in-out"/>
        <div className="order-2 mr-3 text-[#F69838] flex justify-evenly gap-4 items-center">
        {!login && <><IoPersonCircleSharp className="h-6 w-6 hover:scale-105 transition-transform duration-300 ease-in-out"/></>}
        {!login && <><FaShoppingCart className="h-6 w-6 hover:scale-105 transition-transform duration-300 ease-in-out"/></>}
        <div onClick={handlefeats}><FaBars className="h-6 w-6 hover:scale-105 transition-transform duration-300 ease-in-out"/>
        </div>
        ...
        {mobile && showfeat && 
  <div id="disp" ref={dispRef} className="h-fit p-3 w-[250px] rounded-lg bg-[#4B4B4B] absolute top-[70px] right-3">
    <div className="flex gap-3 justify-evenly items-center">
    {!login && first.map((item, index) => (
               <a
    ...
    <div className="flex flex-col fontfam p-3 space-y-1 relative">
      ...
      {NAV_ITEMS.map((item, index) => (
        <div key={index} className="relative">
          <a className="hover:scale-105 hover:text-[orange] transition duration-300 ease-in-out py-2 flex justify-start gap-3 items-center" href={item.href}>
            {item.tag}
            {item.label}
          </a>
      ...
      {!mobile && <div className="flex justify-between w-full fontfam m-0 p-0">
        <div className="flex pl-3 gap-[3px] mt-2 order-1">{basic.map((item, index) => (
                <a className={`hover:scale-105 p-3 hover:bg-gradient-to-b h-[60px] from-[orange] hover:text-[#1A1A40] w-[100px] flex justify-center items-center ${
                 index === 0 ? 'module' : ''
               } text-[#F69838]`} key={index} href={item.href}>
                  
                  {item.label}

              </a>
            ))}</div><img src="/logo.png" alt="Meesho" className=" h-[80px] w-[220px] md:h-[7pc] md:w-[19pc] p-5 hover:scale-105 transition-transform duration-300 ease-in-out order-2"/>
        <div className="flex pr-3  gap-[3px] md:order-3 order-2">{login && !mobile && first.map((item, index) => (
               <a
