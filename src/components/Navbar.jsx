  ...
  return (
    <nav className="flex flex-col justify-center text-xl items-center navy-blue text-[ivory]">
      <div className="flex justify-between w-full fontfam m-0 p-0">
        <div className="flex pl-3 md:gap-[3px] gap-[2px] mt-2">
          {basic.map((item, index) => (
            <a
              className={
                mobile
                  ? `hover:bg-[orange] text-[#1A1A40]
        ...
        <img
          src="/logo.png"
          alt="Meesho"
          className="h-[80px] w-[220px] md:h-[7pc] md:w-[19pc] p-5 hover:scale-105 transition-transform duration-300 ease-in-out"
        />
        <div className="flex pr-3  gap-[3px]">
          {mobile && (
            <button className="join-button font-playfair text-[#1A1A40] font-bold px-4 border border-transparent rounded-md transition duration-300 ease-in-out h-[40px] w-[80px] my-2">
              Join
            </button>
          )}
          {!login &&
          ...
          {login &&
            second.map((item, index) => (
              <a
                className={`hover:scale-105 p-3 hover:bg-gradient-to-b h-[60px] hover:from-[orange] w-[110px] flex justify-center items-center gap-2 ${
                  index === 0 ? "module" : ""
                }`}
                key={index}
                href={item.href}
